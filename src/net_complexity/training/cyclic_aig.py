...
def _extract_layer_g_probs(valid_metrics: Mapping[str, Any]) -> dict[str, float]:
    ...
    for key, value in valid_metrics.items():
        ...
        # Skip aggregate keys: average_prob, max_prob, min_prob, etc.
...
def _build_cycle_config(
    ...
    if aig_epochs is not None:
        _set_num_epochs(cfg, int(aig_epochs))
...
def _build_final_config(
    ...
    if final_epochs is not None:
        _set_num_epochs(cfg, int(final_epochs))

    # Disable AIG regularization for purely CE-driven final training.
    ...
    # Optionally swap AIGBottleneckLayer to standard Bottleneck so the gate
