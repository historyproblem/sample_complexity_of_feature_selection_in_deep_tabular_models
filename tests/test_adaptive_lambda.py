...
def test_lambda_increase_warmup_increases_every_configured_epochs_before_adaptive_logic():
    model = _DummyModel(lambda_coef=1.0e-4)
    optimizer = torch.optim.SGD(model.parameters(), lr=0.1)
    log_step = math.log(1.1)
    controller = AdaptiveLambdaController(
        initial_lambda_coef=1.0e-4,
        reference_accuracy_by_epoch={1: 0.90, 2: 0.90, 3: 0.90, 4: 0.90, 5: 0.90},
        warmup_epochs=0,
        lambda_increase_warmup_epochs=4,
        lambda_increase_warmup_every_epochs=2,
        update_every_epochs=2,
        acc_window=1,
        log_step_init=log_step,
        log_step_min=math.log(1.05),
    ...
    lambda_after_first_warmup_update = model.lambda_coef
    third_result = _run_epoch(
        controller,
        model,
        optimizer,
        epoch=3,
    ...
    lambda_after_second_warmup_update = model.lambda_coef
    fifth_result = _run_epoch(
        controller,
        model,
        optimizer,
        epoch=5,
    ...
    assert first_result.action == "warmup_hold"
    assert first_result.metrics["adaptive_lambda_step_action"] == "step_warmup_hold"
    assert second_result.action == "increase_lambda_warmup"
    assert second_result.metrics["adaptive_lambda_step_action"] == (
        "step_warmup_fixed_increase"
    )
    assert second_result.metrics["lambda_increase_warmup_epochs"] == 4
    assert second_result.metrics["lambda_increase_warmup_every_epochs"] == 2
    assert lambda_after_first_warmup_update == pytest.approx(1.0e-4 * 1.1)

    assert third_result.action == "warmup_hold"
    assert fourth_result.action == "increase_lambda_warmup"
    assert lambda_after_second_warmup_update == pytest.approx(1.0e-4 * 1.1**2)

    assert fifth_result.action == "hold"
    assert sixth_result.action == "decrease_lambda"
    assert model.lambda_coef == pytest.approx(lambda_after_second_warmup_update / 1.1)
